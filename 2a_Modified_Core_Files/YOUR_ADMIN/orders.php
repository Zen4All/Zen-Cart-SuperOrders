...
// END TY TRACKER 1 - DEFINE TRACKING VALUES, INCLUDE DATABASE FIELDS IN STATUS QUERY, & E-MAIL TRACKING INFORMATION  ----------------------------------------------
            //send emails
            $message = //<!-- Begin Ty Package Tracker Modification (Minor formatting change) //-->
...
// BEGIN STOCK COMMENTS
        ...
        if (($check_status->fields['orders_status'] != $status) || zen_not_null($comments)) {
          ...
          if (isset($_POST['notify']) && ($_POST['notify'] == '1')) {
            ...
            //send emails
            $message = //<!-- Begin Super Orders Modification (Minor formatting change) //-->
                STORE_NAME . " " . EMAIL_TEXT_ORDER_NUMBER . ' ' . $oID . "\n\n" .
              ...
              <tbody>
                  ...
                  if ($so->payment) {
                    for ($a = 0; $a < sizeof($so->payment); $a++) {
                      ...
                      ?>
                    <tr class="paymentRow" onclick="popupWindow('<?php echo zen_href_link(FILENAME_SUPER_PAYMENTS, 'oID=' . $so->oID . '&payment_mode=payment&index=' . $so->payment[$a]['index'] . '&action=my_update', 'NONSSL'); ?>', 'scrollbars=yes,resizable=yes,width=400,height=300,screenX=150,screenY=100,top=100,left=150')">
